...
def profile():
    if 'loggedin' in session:
        ...
        return render_template('site/profile.html', tab="profile", designation=designation, data=d, user=user,
                               pic=pic_url)
...
def edit_profile():
    if 'loggedin' in session:
        ...
        return render_template('reg-login/edit_profile.html', tab="profile", designation=designation, data=d, user=user,
                               pic=pic_url)
    return redirect(url_for('home'))
    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST':
            ...
            if (check_pass == hash_password.encode('utf8')):
                ...
                msg = '*Incorrect password!'
                return render_template('reg-login/change_pass.html', tab="profile", designation=designation, msg=msg,
                                       user=user)
            mysql.connection.commit()
            return redirect(url_for('profile'))
...
def technicians():
    if 'EmpAccess' in session and session['designation'] == 'MANAGER':
        ...
        count = cur.fetchall()
        # print(data)
        return render_template('employee/technician/technicians.html', flag={'value': 'False'}, count=count,
                               tab="tickets", data=data, user=user, desg=session['designation'])
    return redirect(url_for('emp'))
...
def technician_profile(tech_id):
    ...
    return render_template('/employee/read_profile.html', data=cur.fetchone(), tab="tickets",
...
def assign_job():
    if 'EmpAccess' in session and session['designation'] == 'MANAGER':
        ...
        cur.execute("""select *, count(*) from employee, assignment where employee.employee_id=assignment.employee_id group 
                by employee.employee_id""")
        count = cur.fetchall()
        return render_template('employee/technician/assign_job.html', tab="TECHNICIAN", flag={'value': 'False'},
                               count=count, data=data, user=user, desg=session['designation'])
    return redirect(url_for('emp'))
...
def managers():
    if 'EmpAccess' in session and session['designation'] == 'ADMIN':
        ...
        count = cur.fetchall()
        # print(data)
        return render_template('employee/manager/managers.html', flag={'value': 'False'}, count=count, tab="manager",
                               data=data, user=user, desg=session['designation'])
    return redirect(url_for('emp'))
...
def allocate(mgr_id):
    if 'EmpAccess' in session and session['designation'] == 'ADMIN':
        ...
        emp = cur.fetchone()
        return render_template('employee/manager/allocation.html', tab="manager", user=user, emp=emp,
...
def services():
    if 'loggedin' in session and session['designation'] == "customer":
        ...
        designation = escape(session['designation'])
        cur = mysql.connection.cursor()
        cur.execute("SELECT ticket_id,app_date,app_type,status FROM ticket where user_id=%s ORDER BY curr_date DESC",
                    (user,))
        data = cur.fetchall()
...
def ticket():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            pass
    gc.collect()
    return render_template('forms/ticket.html', tab="services", designation=escape(session['designation']), data=data,
                           date=date.today(),
                           user=escape(session['id']))
...
def all_tickets():
    if 'EmpAccess' in session and session['designation'] == 'EXECUTIVE':
        ...
        data = cur.fetchall()
        return render_template('employee/ticket/all_tickets.html', tab="tickets", data=data, user=user,
                               desg=session['designation'])
    return redirect(url_for('emp'))
...
def assign_pending_inventory(tech_id):
    if 'EmpAccess' in session:
        ...
        emp = cur.fetchone()
        return render_template('employee/ticket/pending_inventory.html', tab="inventory", pending=pending,
            ...
            try:
                cur.execute("""INSERT INTO inventory(ticket_id, product_name,product_type, product_description, fault_type, 
                    ...
                    values(%s,%s,%s,%s,%s,%s,%s)""",
                            (ticket_id, product_name, product_type, product_description, fault_type, fault_description,
                             curr_date))
...
def emp_profile():
    if 'EmpAccess' in session:
        ...
        return render_template('employee/profile/emp_profile.html', tab="profile", desg=session['designation'], data=d,
                               user=user, pic=pic_url)
    return redirect(url_for('emp'))
...
def edit_emp_profile():
    if 'EmpAccess' in session:
        ...
        if request.method == 'POST':
            ...
            try:
                cur.execute(
                    """update employee set fname=%s,lname=%s,phone=%s,address=%s,pincode=%s where employee_id=%s""", (
                        fname, lname, phone, address, pincode, session['id'],))
                if file.filename != '':
                    firebase = pyrebase.initialize_app(config)
                    storage = firebase.storage()
        ...
        return render_template('employee/profile/emp_edit_profile.html', tab="profile", data=d, user=user, pic=pic_url,
                               desg=session['designation'])
    return redirect(url_for('emp'))
...
def change_emp_password():
    if 'EmpAccess' in session:
        ...
        if request.method == 'POST':
            ...
            if (check_pass == hash_password.encode('utf8')):
                new_hash = bcrypt.hashpw(password.encode('utf8'), bcrypt.gensalt())
                cur.execute("""update employee set hash_password=%s where employee_id = %s""",
                            (new_hash, session['id'],))

            else:
                msg = '*Incorrect password!'
                return render_template('employee/profile/emp_change_pass.html', tab="profile",
                                       desg=session['designation'], msg=msg, user=user)
            mysql.connection.commit()
            return redirect(url_for('emp_profile'))
        return render_template('employee/profile/emp_change_pass.html', tab="profile", user=user,
                               desg=session['designation'])
    return redirect(url_for('emp'))
...
def super_panel():
    if 'SuperuserAccess' in session:
        if request.method == 'POST':
            ...
            mysql.connection.commit()
        return render_template('employee/superuser/superuser_panel.html', desg="SUPERUSER",
                               log=session['SuperuserAccess'])
